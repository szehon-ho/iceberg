...
public class TestMetadataTableFilters extends TableTestBase {
  ...
  public void setupTable() throws Exception {
    ...
    if (type.equals(MetadataTableType.ALL_DATA_FILES)) {
      // Clear all files from current snapshot to test 'all' files tables can see previous files
  ...
  private int expectedCount(int partitions) {
    switch (type) {
      case FILES: // one data file/manifest per partition
        return partitions;
      case DELETE_FILES: // one delete file/manifest per partition
        return partitions;
      case ALL_DATA_FILES: // returns the data file/manifest in DELETED and ADDED states
        return partitions * 2;
      default:
        throw new IllegalArgumentException("Unsupported metadata table type:" + type);
    }
