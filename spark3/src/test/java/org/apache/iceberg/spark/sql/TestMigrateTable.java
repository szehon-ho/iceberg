...
public class TestMigrateTable extends SparkCatalogTestBase {

  public TestMigrateTable(String catalogName, String implementation, Map<String, String> config) {
  ...
  public void testMigrateWithOptions() throws IOException {
    ...
    sql("MIGRATE TABLE %s.%s USING iceberg TBLPROPERTIES ('foo' 'bar') ",
  ...
  @Test
  public void testForbidMigrateDifferentForamt() throws IOException {
    Assume.assumeTrue(catalogName.equals("spark_catalog"));
    String location = temp.newFolder().toString();
    sql("CREATE TABLE %s (id bigint NOT NULL, data string) USING parquet LOCATION '%s'", tableName, location);
    sql("INSERT INTO TABLE %s VALUES (1, 'a')", tableName);
    AssertHelpers.assertThrows("Should forbid using anything but iceberg in FORMAT",
        IllegalArgumentException.class,
        () -> scalarSql("MIGRATE TABLE %s.%s USING parquet", catalogName, tableName));
  }
