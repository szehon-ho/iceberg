...
public class TestCTASEncryption extends SparkCatalogTestBase {
  public TestCTASEncryption(String catalogName, String implementation, Map<String, String> config) {
    super(catalogName, implementation, config);
  }

  @Before
  public void createTables() throws IOException {
    sql("CREATE TABLE %s (id bigint, data string, float float) USING iceberg ", tableName + "1");
    sql(
    ...
    sql(
        "CREATE TABLE %s USING iceberg "
            ...
            + " AS SELECT * from %s",
        tableName, MockKMS.MASTER_KEY_NAME1, tableName + "1");
  }
