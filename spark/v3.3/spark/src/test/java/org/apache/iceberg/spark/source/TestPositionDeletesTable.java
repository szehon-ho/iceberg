...
public class TestPositionDeletesTable extends SparkCatalogTestBase {
  ...
  public void testWriteUnpartitionedNullRows() throws Exception {
    ...
    // Compare values without 'delete_file_path' as these have been rewritten
    StructLikeSet actual = actual(tableName, tab, null, ImmutableList.of("file_path", "pos", "row", "spec_id"));

    List<PositionDelete<?>> expectedDeletes =
        Lists.newArrayList(positionDelete(dFile.path(), 0L), positionDelete(dFile.path(), 1L));
    StructLikeSet expected =
        expected(tab, expectedDeletes, null, null);
  ...
  public void testWritePartitionEvolutionAdd() throws Exception {
    ...
    try (CloseableIterable<ScanTask> tasks = posDeletesTable.newBatchScan().filter(
        Expressions.isNull("partition.data")
    ).planFiles()) {
      String fileSetID = UUID.randomUUID().toString();
      stageTask(tab, fileSetID, tasks);
    ...
    Assert.assertEquals("Position Delete table should contain expected rows", expectedUnpartitioned, actualUnpartitioned);

    // Read/write back new partition spec (data)
    try (CloseableIterable<ScanTask> tasks =
             CloseableIterable.filter(posDeletesTable.newBatchScan().planFiles(),
    ...
    Record partitionA = partitionRecordTemplate.copy("data", "a");
    Record partitionB = partitionRecordTemplate.copy("data", "b");
    StructLikeSet expectedAll = StructLikeSet.create(
        TypeUtil.selectNot(
            posDeletesTable.schema(), ImmutableSet.of(MetadataColumns.FILE_PATH_COLUMN_ID)).asStruct());
    ...
    StructLikeSet actualAll = actual(tableName, tab, "partition.data = 'a' OR partition.data = 'b'", NON_PATH_COLS);
    Assert.assertEquals("Position Delete table should contain expected rows", expectedAll, actualAll);
  ...
  public void testWriteSchemaEvolutionAdd() throws Exception {
    ...
    StructLikeSet expectedA =
        expected(tab, expectedDeletesA, partitionA, null);
    ...
    Record partitionC = partitionRecordTemplate.copy("data", "c");
    StructLikeSet expectedC =
        expected(tab, deletesC.first(), partitionC, null);
  ...
  public void testNormalWritesNotAllowed() throws IOException {
    ...
    Assert.assertThrows("position_deletes table can only be written by RewriteDeleteFiles",
  ...
  private void commit(Table baseTab, Table posDeletesTable, String fileSetID, int expectedFileSize) {
    ...
    List<String> sortedAddedFiles = addedFiles.stream().map(f -> f.path().toString()).sorted().collect(Collectors.toList());
    List<String> sortedRewrittenFiles = rewrittenFiles.stream().map(f -> f.path().toString()).sorted().collect(Collectors.toList());
